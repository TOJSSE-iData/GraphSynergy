...
class RegressTrainer(BaseTrainer):
    def __init__(self, 
                 model, 
                 criterion, 
                 metric_fns, 
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(self.valid_data_loader):
                ...
                self.writer.set_step((epoch - 1) * len(self.valid_data_loader) + batch_idx, 'valid')
                self.valid_metrics.update('loss', loss.item())
                y_pred = output.flatten().cpu().detach().numpy().tolist()
                y_true = target.flatten().cpu().detach().numpy().tolist()
                y_preds.extend(y_pred)
                y_trues.extend(y_true)
        for met in self.metric_fns:
            self.valid_metrics.update(met.__name__, met(y_preds, y_trues))
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(self.test_data_loader):
                ...
                y_trues.extend(y_true)
        for met in self.metric_fns:
            total_metrics[met.__name__] = met(y_preds, y_trues)
